...
且列舉報告應載明結果僅依賴詞法抽取。
該次執行不得使用「已驗證」一詞。這是層級 B。

若根本無法查詢列舉 —— 此環境無法執行任何指令 —— 該次執行屬於層級 C。
列舉是「讀」出來的，報告應載明 shared/verification-tiers.md 中的揭露，
指名「讀」查不到的三件事：遞移繼承、原始碼樹之外的基底類別、
以及反射註冊。
在本技能組自己的 fixture 上，純文字搜尋只找到 6 個交易類別中的 3 個。
